...
func ReplaceFirstSpaceRuneWithEmptyString(str string) string {
	if string([]rune(str)[:1]) == " " {
		return string([]rune(str)[:0]) + "" + string([]rune(str)[1:])
	}
	return str
}

func SanitizePercentageString(str string) string {
	if str == "%" {
		return "%25"
	}
	return str
